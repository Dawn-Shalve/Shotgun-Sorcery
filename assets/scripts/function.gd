...
func load_screen_to_scene(target: String, parameters: Dictionary = {}) -> void:
	var loading_screen = preload("res://assets/scenes/loadingscreen.tscn").instantiate() # Godot handles loading the loading screen upon game startup. We'll instance a new one here.
...
func explode(position: Vector2):
	var parti = load("res://assets/scenes/parti.tscn").instantiate
	parti.position = position
	add_child(parti)
